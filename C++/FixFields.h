...
namespace FIX
{
  ...
  DEFINE_INT(LastLiquidityIndicator);
  ...
  DEFINE_INT(NoOrderAttributes);
  DEFINE_INT(OrderAttributeType);
  DEFINE_STRING(OrderAttributeValue);
  DEFINE_UTCTIMESTAMP(OrderEnteredTime);
  DEFINE_STRING(SelfMatchPreventionID);
  DEFINE_CHAR(SMPInstruction);
  DEFINE_INT(TrdRegPublicationReason);
  DEFINE_STRING(TradingVenueRegulatoryTradeID);
  DEFINE_INT(ExpectedFillType);
  DEFINE_STRING(SecurityAltID);
  DEFINE_STRING(UnderlyingSecurityAltID);
  DEFINE_STRING(SecondaryExecID);
  DEFINE_STRING(FixingDate);
  DEFINE_STRING(TTUsername);
  DEFINE_PRICE(LegPrice);
  DEFINE_STRING(SecuritySubType);
  DEFINE_INT(TrdType);
  DEFINE_BOOLEAN(ManualOrderIndicator);
  DEFINE_STRING(OrigExecID);
  DEFINE_STRING(TTSessionID);
  DEFINE_STRING(SyntheticOrdStatus);
  DEFINE_STRING(UserTag);
  DEFINE_STRING(OrderTag);
  DEFINE_STRING(StagedOrderMsg);
  DEFINE_STRING(SyntheticOrdType);
  DEFINE_STRING(SyntheticOrdMod);
  DEFINE_CHAR(StagedOrderStatus);
  DEFINE_STRING(StagedOrderOwner);
  DEFINE_CHAR(StagedRoutingLevel);
  DEFINE_STRING(FFT2);
  DEFINE_STRING(FFT3);
  DEFINE_INT(NoLinks);
  DEFINE_STRING(LinkID);
  DEFINE_CHAR(LinkType);
  DEFINE_STRING(RelativePriceType);
  DEFINE_INT(RelativePriceOffset);
  DEFINE_UTCTIMESTAMP(StartDateTime);
  DEFINE_UTCTIMESTAMP(EndDateTime);
  DEFINE_STRING(EndDateTimeAction);
  DEFINE_STRING(WATType);
  DEFINE_INT(WATValue);
  DEFINE_INT(MGNumRetries);
  DEFINE_INT(MGRetryInterval);
  DEFINE_PRICE(TriggerPrice);
  DEFINE_CHAR(TriggerPriceType);
  DEFINE_STRING(TriggerQtyType);
  DEFINE_STRING(TriggerQtyOperator);
  DEFINE_INT(TriggerQty);
  DEFINE_INT(TrailingOffset);
  DEFINE_STRING(TrailingPriceType);
  DEFINE_STRING(OrderOriginationID);
  DEFINE_STRING(ExchangeOrderID);
  DEFINE_INT(DisclosedQtyValue);
  DEFINE_INT(DisclosedQtyVariance);
  DEFINE_INT(SlicerIntervalValue);
  DEFINE_STRING(SlicerIntervalUnit);
  DEFINE_STRING(DurationType);
  DEFINE_INT(DurationValue);
  DEFINE_STRING(DurationUnit);
  DEFINE_UTCTIMESTAMP(DurationBeginTime);
  DEFINE_UTCTIMESTAMP(DurationEndTime);
  DEFINE_STRING(LeftActionTrigger);
  DEFINE_STRING(LeftAction);
  DEFINE_INT(LeftPayupTicks);
  DEFINE_INT(MarketOrderLimit);
  DEFINE_CURRENCY(RealizedPandL);
  DEFINE_STRING(ExchContractSymbol);
  DEFINE_INT(PriceDisplayType);
  DEFINE_FLOAT(TickSize);
  DEFINE_FLOAT(PointValue);
  DEFINE_STRING(SecurityAltID_ISIN);
  DEFINE_INT(NumTickTblEntries);
  DEFINE_INT(NumTicks);
  DEFINE_PRICE(MaxPrice);
  DEFINE_INT(DeliveryUnit);
  DEFINE_INT(LotSize);
  DEFINE_INT(Blocks);
  DEFINE_CHAR(TradesInFlow);
  DEFINE_CHAR(PassiveAggressive);
  DEFINE_BOOLEAN(AutoAggressive);
  DEFINE_INT(MDEntryState);
  DEFINE_STRING(MDEntrySizeType);
  DEFINE_CHAR(AggressorSide);
  DEFINE_CHAR(ExactNumOrdersIndicator);
  DEFINE_FLOAT(ExchTickSize);
  DEFINE_FLOAT(ExchPointValue);
  DEFINE_BOOLEAN(DropCopyOrder);
  DEFINE_CHAR(LegSide);
  DEFINE_STRING(PosReqId);
  DEFINE_STRING(PosMaintRptId);
  DEFINE_INT(PosReqType);
  DEFINE_INT(TotalNumPosReports);
  DEFINE_INT(TotalNumOrders);
  DEFINE_STRING(FFT4);
  DEFINE_STRING(FFT5);
  DEFINE_STRING(FFT6);
  DEFINE_BOOLEAN(RequestTickTable);
  DEFINE_INT(ForceLogout);
  DEFINE_STRING(GatewayStatusReqId);
  DEFINE_INT(NoGatewayStatus);
  DEFINE_INT(GatewayStatus);
  DEFINE_STRING(ExchangeGateway);
  DEFINE_INT(SubExchangeGateway);
  DEFINE_STRING(TTAccountType);
  DEFINE_INT(NoGateways);
  DEFINE_STRING(TTSecurityAlias);
  DEFINE_STRING(TTStrategyEngine);
  DEFINE_INT(PriceFeedStatus);
  DEFINE_CHAR(DeliveryTerm);
  DEFINE_CHAR(UnderlyingDeliveryTerm);
  DEFINE_CHAR(IncludeHistoricalFills);
  DEFINE_CHAR(IncludeNumberOfOrders);
  DEFINE_STRING(ExchCred);
  DEFINE_STRING(BrokerID);
  DEFINE_STRING(CompanyID);
  DEFINE_STRING(TTCustomerName);
